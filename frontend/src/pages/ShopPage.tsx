...
import { Check, Heart, Image, Lock, Music, Star } from "lucide-react";
...
export default function ShopPage() {
  ...
  const [unlockedBgs, setUnlockedBgs] = useState<
    BackgroundUnlockedResponseDTO[]
  >([]);
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6">
        {(
          ...
          ] as { key: Tab; label: string; icon: typeof Music }[]
              ...
              >
                ...
                <div className="flex gap-2 mt-auto">
                  {!unlocked ? (
                    <button
                      onClick={() =>
                        handleUnlockMusic(item.id, item.requiredPoint)
                      }
                    ...
                    <button
                      onClick={
                        saved
                          ? () => handleRemoveSavedMusic(item.id)
                          : () => handleSaveMusic(item.id)
                      }
                      disabled={isBusy}
                      className={clsx(
                ...
                <div className="p-3 flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-1.5">
                    {!unlocked ? (
                      <button
                        onClick={() =>
                          handleUnlockBg(item.id, item.requiredPoint)
                        }
                        disabled={isBusy || !canAfford}
                      ...
                      <button
                        onClick={
                          saved
                            ? () => handleRemoveSavedBg(item.id)
                            : () => handleSaveBg(item.id)
                        }
                        disabled={isBusy}
                        className={clsx(
